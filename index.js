...
async function ensureBase64Images(content) {
      ...
      if (url && url.startsWith('http')) {
        try {
          ...
          result.push({
            ...
            image_url: {
              url: base64Url,
              detail: (item.image_url && item.image_url.detail) || 'auto',
            }
          });
        } catch (err) {
          result.push({
  ...
  try {
    ...
    for (const msg of history) {
      sanitizedHistory.push({
        role: msg.role,
        content: await ensureBase64Images(msg.content)
      });
    }
    ...
    const finalUserMessage = normalizeRoleContent(
      "user",
      await ensureBase64Images(userContent)
    );
...
แถมแปรงสีฟันพรี่เมี่ยม 1 แพ็ก ถึง 6 ด้าม (และยาสีฟัน 4 หลอด)
